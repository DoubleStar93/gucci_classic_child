{**
 * Classic Gucci — totali carrello (override)
 * Spedizione: GucciCartShipping via hook displayShoppingCart (fissa + soglia gratuita).
 *}
  ...
  <div class="card-block cart-summary-subtotals-container js-cart-summary-subtotals-container">
    ...
    {foreach from=$cart.subtotals item="subtotal"}
      {if $subtotal && $subtotal.type !== 'tax'}
        {assign var='gucciShowSubtotal' value=false}
        {if $subtotal.type === 'shipping'}
          {assign var='gucciShowSubtotal' value=true}
        {elseif $subtotal.value|count_characters > 0}
          {assign var='gucciShowSubtotal' value=true}
        {/if}

        {if $gucciShowSubtotal}
          ...
          <div class="cart-summary-line" id="cart-subtotal-{$subtotal.type}">
            ...
            <span class="value">
              ...
              {if $subtotal.type === 'shipping' && isset($gucci_shipping_value) && $gucci_shipping_value|count_characters > 0}
                {$gucci_shipping_value}
              {elseif $subtotal.type === 'shipping' && isset($gucci_shipping_amount) && $gucci_shipping_amount <= 0 && $subtotal.value|count_characters > 0}
                {$subtotal.value}

...
class BarbaraalvisiCartShipping
{
    /**
    ...
    public static function assignCartShippingVars(Cart $cart, Context $context): void
    {
        try {
            ...
            $context->smarty->assign([
                'barbaraalvisi_shipping_amount' => $shipping['amount'],
                'barbaraalvisi_shipping_value' => $shipping['value'],
                'barbaraalvisi_shipping_label' => $shipping['label'],
                'barbaraalvisi_cart_total_amount' => $total['amount'],
                'barbaraalvisi_cart_total_value' => $total['value'],
                'barbaraalvisi_free_shipping_show' => $freeShipping['show'],
                'barbaraalvisi_free_shipping_remaining_amount' => $freeShipping['remaining_amount'],
                'barbaraalvisi_free_shipping_remaining_value' => $freeShipping['remaining_value'],
                'barbaraalvisi_free_shipping_reached' => $freeShipping['reached'],
                'barbaraalvisi_free_shipping_threshold_amount' => $freeShipping['threshold_amount'],
            ]);
        } catch (Throwable $exception) {
            PrestaShopLogger::addLog(
                'BarbaraalvisiCartShipping: ' . $exception->getMessage(),
                3
            );
        }
    }
    ...
    public static function getStoreFreeShippingThreshold(): float
    {
        ...
        $configured = (float) Configuration::get('BARBARAALVISI_FREE_SHIPPING_THRESHOLD');
        if ($configured > 0) {
            return $configured;
        }

{**
 * Barbara Alvisi — aside informativo pagina Contattaci (override ps_contactinfo-rich)
 *}
<div class="barbaraalvisi-contact-aside-panel">
  {if isset($contact_infos.company) && $contact_infos.company}
    <p class="barbaraalvisi-contact-aside__brand">{$contact_infos.company}</p>
  {/if}

  {if isset($contact_infos.email) && $contact_infos.email && $display_email}
    <div class="barbaraalvisi-contact-aside__block">
      <h2 class="barbaraalvisi-contact-aside__label">
        {if $language.iso_code == 'it'}E-mail{else}{l s='Email' d='Shop.Theme.Global'}{/if}
      </h2>
      <a href="mailto:{$contact_infos.email|escape:'url'}" class="barbaraalvisi-contact-aside__link">
        {$contact_infos.email}
      </a>
      <p class="barbaraalvisi-contact-aside__note">
  ...
  {if isset($contact_infos.phone) && $contact_infos.phone}
    ...
    <div class="barbaraalvisi-contact-aside__block">
      ...
      <a href="tel:{$barbaraalvisi_phone_href}" class="barbaraalvisi-contact-aside__link">
        {$contact_infos.phone}
      </a>
    </div>
  {/if}
  ...
  {if isset($contact_infos.address) && ($contact_infos.address.city || $contact_infos.address.country)}
    <div class="barbaraalvisi-contact-aside__block">
      ...
      <address class="barbaraalvisi-contact-aside__address">

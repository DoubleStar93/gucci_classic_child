{**
 * Barbara Alvisi — riepilogo articoli conferma ordine
 *}
<div class="barbaraalvisi-order-confirmation-table-wrap">
  ...
  <div class="order-confirmation-table barbaraalvisi-order-confirmation-lines">
    {foreach from=$products item=product}
      <article class="order-line barbaraalvisi-order-confirmation-line">
        <div class="barbaraalvisi-order-confirmation-line__media">
        ...
        <div class="barbaraalvisi-order-confirmation-line__body">
          <h3 class="barbaraalvisi-order-confirmation-line__name">
            {if $add_product_link}<a href="{$product.url}">{$product.name}</a>{else}{$product.name}{/if}
          </h3>
          <p class="barbaraalvisi-order-confirmation-line__unit">
            {if $language.iso_code == 'it'}Prezzo unitario{else}{l s='Unit price' d='Shop.Theme.Checkout'}{/if}:
            <span>{$product.price}</span>
          </p>
        </div>

        <div class="barbaraalvisi-order-confirmation-line__qty" aria-label="{l s='Quantity' d='Shop.Theme.Checkout'}">
          {$product.quantity}
        </div>

        <div class="barbaraalvisi-order-confirmation-line__total">
          {$product.total}
        </div>
      </article>
    {/foreach}
  ...
  <div class="barbaraalvisi-order-confirmation-totals">
    {foreach $subtotals as $subtotal}
      {if $subtotal !== null && $subtotal.type !== 'tax' && $subtotal.label !== null}
        <div class="cart-summary-line barbaraalvisi-order-confirmation-summary-line" id="order-subtotal-{$subtotal.type}">
          <span class="label">
    ...
    {if !$configuration.display_prices_tax_incl && $configuration.taxes_enabled}
      <div class="cart-summary-line barbaraalvisi-order-confirmation-summary-line">
        <span class="label">{$totals.total.label}&nbsp;{$labels.tax_short}</span>
        <span class="value">{$totals.total.value}</span>
      </div>
      <div class="cart-summary-line cart-total barbaraalvisi-order-confirmation-summary-line barbaraalvisi-order-confirmation-summary-line--total">
    ...
    {if $subtotals.tax !== null && $subtotals.tax.label !== null}
      <div class="cart-summary-line barbaraalvisi-order-confirmation-summary-line barbaraalvisi-order-confirmation-summary-line--tax">

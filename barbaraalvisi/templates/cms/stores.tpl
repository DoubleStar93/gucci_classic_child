...
{block name='page_content_container'}
  <section id="content" class="page-content page-stores barbaraalvisi-stores-page barbaraalvisi-page-content">
    {if $stores|@count == 0}
      <p class="barbaraalvisi-stores-empty">
    ...
    {foreach $stores as $store}
      <article id="store-{$store.id|intval}" class="store-item barbaraalvisi-store-card">
        <div class="barbaraalvisi-store-card-inner">
          {if $store.image}
            <div class="barbaraalvisi-store-image hidden-sm-down">
              <picture>
          ...
          <div class="barbaraalvisi-store-details">
            <h2 class="barbaraalvisi-store-name">{$store.name|escape:'htmlall':'UTF-8'}</h2>
            <address class="barbaraalvisi-store-address">{$store.address.formatted nofilter}</address>

            {if $store.note || $store.phone || $store.fax || $store.email}
              <button
                type="button"
                class="barbaraalvisi-store-toggle btn-unstyle"
            ...
            {if $store.business_hours}
              <div class="barbaraalvisi-store-hours">
                <p class="barbaraalvisi-store-hours-title">
                  {if $language.iso_code == 'it'}Orari{else}{l s='Opening hours' d='Shop.Theme.Global'}{/if}
                </p>
                <table class="barbaraalvisi-store-hours-table">
        ...
        {if $store.note || $store.phone || $store.fax || $store.email}
          <footer id="about-{$store.id|intval}" class="barbaraalvisi-store-footer collapse">
            {if $store.note}
              <p class="barbaraalvisi-store-note">{$store.note|escape:'htmlall':'UTF-8'}</p>
            {/if}
            <ul class="barbaraalvisi-store-contacts">
              {if $store.phone}

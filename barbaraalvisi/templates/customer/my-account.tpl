...
{block name='page_content'}
  ...
  <nav class="barbaraalvisi-account-links" aria-label="{l s='Your account' d='Shop.Theme.Customeraccount'}">
    <a class="barbaraalvisi-account-link" id="identity-link" href="{$urls.pages.identity}">
      {if $language.iso_code == 'it'}Informazioni personali{else}{l s='Information' d='Shop.Theme.Customeraccount'}{/if}
    </a>

    {if $customer.addresses|count}
      <a class="barbaraalvisi-account-link" id="addresses-link" href="{$urls.pages.addresses}">
        {if $language.iso_code == 'it'}Indirizzi{else}{l s='Addresses' d='Shop.Theme.Customeraccount'}{/if}
      </a>
    {else}
      <a class="barbaraalvisi-account-link" id="address-link" href="{$urls.pages.address}">
        {if $language.iso_code == 'it'}Aggiungi indirizzo{else}{l s='Add first address' d='Shop.Theme.Customeraccount'}{/if}
      </a>
    {/if}

    {if !$configuration.is_catalog}
      <a class="barbaraalvisi-account-link" id="history-link" href="{$urls.pages.history}">
        {if $language.iso_code == 'it'}Ordini{else}{l s='Order history and details' d='Shop.Theme.Customeraccount'}{/if}
      </a>
    {/if}

    {if !$configuration.is_catalog}
      <a class="barbaraalvisi-account-link" id="order-slips-link" href="{$urls.pages.order_slip}">
        {if $language.iso_code == 'it'}Note di credito{else}{l s='Credit slips' d='Shop.Theme.Customeraccount'}{/if}
      </a>
    {/if}

    {if $configuration.voucher_enabled && !$configuration.is_catalog}
      <a class="barbaraalvisi-account-link" id="discounts-link" href="{$urls.pages.discount}">
        {if $language.iso_code == 'it'}Buoni sconto{else}{l s='Vouchers' d='Shop.Theme.Customeraccount'}{/if}
      </a>
    {/if}

    {if $configuration.return_enabled && !$configuration.is_catalog}
      <a class="barbaraalvisi-account-link" id="returns-link" href="{$urls.pages.order_follow}">
...
{block name='page_footer'}
  {block name='my_account_links'}
    <div class="barbaraalvisi-account-logout">

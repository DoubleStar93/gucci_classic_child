...
const tmp = path.join(os.tmpdir(), "fix-front-js-base");
...
// Fix front_js: absolute root URL so admin pretty-routes don't prefix /admin/js
// Symfony UrlPackage via base_urls — avoids /l1ka80lkkixgfknd/js/* 404
const frontJsBlock = `    packages:
      front_js:
        base_urls:
          - '${staging}/js'`;

if (/packages:\s*\n\s*front_js:\s*\n\s*base_path:\s*['"][^'"]+['"]/.test(yml)) {
  yml = yml.replace(
    /packages:\s*\n\s*front_js:\s*\n\s*base_path:\s*['"][^'"]+['"]/,
    frontJsBlock
  );
  ...
  console.error("Impossibile trovare blocco front_js");
  process.exit(1);
}
...
console.log(yml.slice(idx, idx + 280));
...
client.close();
console.log("OK: front_js →", `${staging}/js`);
